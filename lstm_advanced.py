...
class dataset(Dataset):
    ...
    def split_train_test(df, test_size):
        ...
        train = list()
        test = list()
        ratio = len(df[df['song_popularity'] == 1])/ (len(df[df['song_popularity'] == 1])+len(df[df['song_popularity'] == 0]))
        for artist_idx, artist in enumerate(artists):
            ...
            if artist_idx < 650:
                ...
                # df_artist = under_sampling(df_artist, 0.5)
            if df_artist.empty:
                continue
            X = df_artist.copy()
            ...
            if artist_idx < 650:
                train.append((X_chain, Y_chain))
            else:
                test.append((X_chain, Y_chain))
